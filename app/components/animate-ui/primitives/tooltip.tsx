...
function getResolvedSide(placement: Side | `${Side}-${Align}`) {
  if (placement.includes("-")) return placement.split("-")[0] as Side;
...
function TooltipProvider({
  ...
  const showTooltip = React.useCallback(
    (data: TooltipData) => {
      ...
      if (currentTooltip !== null) { setCurrentTooltip(data); return; }
      const now = Date.now();
  ...
  React.useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => { if (e.key === "Escape") hideImmediate(); };
  ...
  return (
    <GlobalTooltipProvider
      value={{ showTooltip, hideTooltip, hideImmediate, currentTooltip, transition, globalId: id ?? globalId, setReferenceEl, referenceElRef }}
    >
...
type RenderedTooltipContextType = { side: Side; align: Align; open: boolean };
const [RenderedTooltipProvider, useRenderedTooltip] =
  getStrictContext<RenderedTooltipContextType>("RenderedTooltipContext");

type FloatingContextType = { context: UseFloatingReturn["context"]; arrowRef: React.RefObject<SVGSVGElement | null> };
...
type TooltipArrowProps = Omit<React.ComponentProps<typeof MotionTooltipArrow>, "context"> & { withTransition?: boolean };
...
function TooltipPortal(props: React.ComponentProps<typeof FloatingPortal>) {
...
function TooltipOverlay() {
  ...
  React.useEffect(() => {
    if (currentTooltip) setRendered({ data: currentTooltip, open: true });
    else setRendered((p) => (p.data ? { ...p, open: false } : p));
  }, [currentTooltip]);

  React.useLayoutEffect(() => {
    if (referenceElRef.current) { refs.setReference(referenceElRef.current); update(); }
      ...
      {rendered.data && ready && (
        <TooltipPortal>
          <div
            ...
            style={{ position: strategy, top: 0, left: 0, zIndex: 50, transform: `translate3d(${x}px, ${y}px, 0)` }}
          >
            <FloatingProvider value={{ context, arrowRef }}>
              <RenderedTooltipProvider value={{ side: resolvedSide, align: rendered.data.align, open: rendered.open }}>
                <Component
                  ...
                  animate={rendered.open ? { opacity: 1, scale: 1, x: 0, y: 0 } : { opacity: 0, scale: 0, ...initialFromSide(rendered.data.side) }}
                  exit={{ opacity: 0, scale: 0, ...initialFromSide(rendered.data.side) }}
                  onAnimationComplete={() => { if (!rendered.open) setRendered({ data: null, open: false }); }}
                  transition={transition}
                  {...rendered.data.contentProps}
                  style={{ position: "relative", ...(rendered.data.contentProps?.style || {}) }}
                />
...
type TooltipProps = { children: React.ReactNode; side?: Side; sideOffset?: number; align?: Align; alignOffset?: number };
...
function Tooltip({ children, side = "top", sideOffset = 0, align = "center", alignOffset = 0 }: TooltipProps) {
  ...
  return (
    <LocalTooltipProvider value={{ props, setProps, asChild, setAsChild, side, sideOffset, align, alignOffset, id }}>
      {children}
...
function TooltipContent({ asChild = false, ...props }: TooltipContentProps) {
  ...
  React.useEffect(() => {
    if (!shallowEqualWithoutChildren(lastPropsRef.current, props)) { lastPropsRef.current = props; setProps(props); }
  }, [props, setProps]);
  React.useEffect(() => { setAsChild(asChild); }, [asChild, setAsChild]);
  return null;
}
...
function TooltipTrigger({ ref, onMouseEnter, onMouseLeave, onFocus, onBlur, onPointerDown, asChild = false, ...props }: TooltipTriggerProps) {
  ...
  const handleMouseEnter = React.useCallback((e: React.MouseEvent<HTMLDivElement>) => { onMouseEnter?.(e); handleOpen(); }, [handleOpen, onMouseEnter]);
